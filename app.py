...
def render_access_gate():
    ...
    with center_col:
        with st.container():
            with st.form("access_form"):
                ...
                contact_opt_out = st.checkbox(
                    TEXTS["contact_opt_out"][lang],
                    value=bool(pending_profile.get("contact_opt_out", False))
                )
                submit = st.form_submit_button(TEXTS["otp_send_button"][lang], use_container_width=True)
    ...
    if submit:
        profile = {
            ...
            "contact_opt_out": bool(contact_opt_out)
        }
    ...
    if "pending_otp_hash" in st.session_state:
        _, otp_col, _ = st.columns([1, 1.2, 1])
        with otp_col:
            otp_input = st.text_input(TEXTS["otp_code_label"][lang], key="otp_input")
            verify_click = st.button(TEXTS["otp_verify_button"][lang], use_container_width=True)
        if verify_click:
...
def main():
    ...
    param_labels = (format_subscript_label("y_i"), format_subscript_label("y_f"))
    rate_label = format_subscript_label("r_max")
                            ...
                            for n in range(1, max_phases + 1):
                                with st.expander(
                                    ...
                                    with st.spinner(TEXTS['optimizing'][lang].format(n)):
                                        ...
                                        should_reuse_single_phase = (n == 1 and model_name == "Boltzmann" and first_order_single_phase_cache is not None)
                                        if should_try_first_order or should_reuse_single_phase:
                                            if n == 1 and first_order_single_phase_cache is not None:
                                                res_first_order = deepcopy(first_order_single_phase_cache["result"])
                                                res_first_order["reused_from_model"] = first_order_single_phase_cache["source_model"]
                                            else:
                                                ...
                                                if n == 1 and res_first_order is not None:
                                            ...
                                            if res_first_order:
                                                ...
                                                if "reused_from_model" in res_first_order:
                                                    st.caption(TEXTS["fit_reused_n1"][lang].format(res_first_order["reused_from_model"]))
                                                display_single_fit(
                                                    res_first_order,
                                                    replicates,
                                                    model_name,
